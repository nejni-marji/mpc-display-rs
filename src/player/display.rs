...
use mpd::{
    message::Channel, search::Window, song::QueuePlace, Client, Idle, Query, Song, State,
    Subsystem, Term,
};
...
impl MusicData {
    ...
    fn update_status(&mut self, client: &Mutex<Client>) {
        ...
        self.ersc_opts = vec![status.repeat, status.random, status.single, status.consume];
    ...
    fn update_song(&mut self, client: &Mutex<Client>) {
        ...
        let album_track = Self::get_metadata(&song, "track").and_then(|t| t.parse().ok());
    ...
    fn update_playlist(&mut self, client: &Mutex<Client>) {
        ...
        if self.options.verbose {
            ...
            for tag in &self.format {
                ...
                let temp1 = queue
                    .first()
                    .map_or_else(|| Some(String::new()), |s| Self::get_metadata(s, tag));
                for song in &queue {
    ...
    fn print_header(&self) -> String {
        ...
        let elapsed_pretty =
            Self::get_pretty_time(self.time_curr).unwrap_or_else(|| UNKNOWN.into());
        let duration_pretty =
            Self::get_pretty_time(self.time_total).unwrap_or_else(|| UNKNOWN.into());

        let percent = match (self.time_curr, self.time_total) {
            (Some(curr), Some(total)) => (100 * curr.as_secs() / total.as_secs()).to_string(),
            _ => UNKNOWN.into(),
        };
    ...
    fn print_queue(&self, height: u32, width: u32, header_height: u32) -> String {
        ...
        let queue = Self::filter_queue(&queue, queue_height, queue_size, song_pos).join("\n");

        // wrap the queue
        let opt = textwrap::Options::new(width.try_into().expect("nothing should be that big"));
        ...
        // filter the queue
        let queue = Self::filter_queue(&queue, queue_height, queue_size, song_pos);
    ...
    #[allow(clippy::let_and_return)]
    fn filter_queue<T>(queue: &[T], queue_height: u32, queue_size: u32, song_pos: u32) -> &[T] {
